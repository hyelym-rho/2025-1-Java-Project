...
public class ClassManager {
    ...
    public LectureListView getLectureListView(JSONObject jsonObject) {
        if (m_lectureListView == null)
            m_lectureListView = new LectureListView();

        m_lectureListView.setLectureList(jsonObject); // 여기서 학점 계산도 하게끔 View 안에서 처리하도록!
    ...
    public DAO getDAO(){
        if (m_DAO == null) {
            m_DAO = new Controller.DAO(); // or 정확한 클래스명
        }
        return m_DAO;
    }
